...
for article in data.find_all('a', class_='Link-module-root Link-module-isInBlockTitle'):
    title = article.get_text(strip=True)
    
    if text in title.lower():
        link = 'https://meduza.io/' + str(article['href'])
        print(title, link)
        links[link] = title
...
try:
    for url1 in links:
        ...
        for article in data1.find_all('p'):
            ...
            # print(itog)
            itog = itog.split('Подробнее про «Сигнал»')[0]
            print(itog)
