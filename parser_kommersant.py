...
for article in data.find_all('a',class_='uho__link uho__link--overlay'):
    title = article.get_text(strip=True)
    
    if text in title.lower():
        link = 'https://www.kommersant.ru' + str(article['href'])
        print(title, link)
        links[link] = title
...
try:
    for url1 in links:
        ...
        for article in data1.find_all('p', class_='doc__text'):
